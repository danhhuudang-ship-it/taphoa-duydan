  ...
  const handle = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'login') {
        ...
        if (error) throw error;
        toast.success('Tạo tài khoản OK — kiểm tra email xác thực nếu cần');
      }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4">
      <motion.div
        ...
        className="w-full max-w-5xl grid md:grid-cols-2 gap-0 glass-strong rounded-3xl overflow-hidden shadow-glow-lg"
      >
        {/* LEFT — branding */}
        <div className="relative p-10 hidden md:flex flex-col justify-between bg-gradient-to-br from-indigo-600/30 via-fuchsia-600/20 to-pink-600/20">
          ...
          <div className="space-y-4">
            <h2 className="text-3xl lg:text-4xl font-bold leading-tight">
              Bán nhanh hơn,<br />báo cáo thông minh hơn.
            </h2>
            <p className="text-slate-300/80 text-sm leading-relaxed">
              Tất cả nghiệp vụ POS — bán hàng tại quầy, kho, khách hàng, công nợ, báo cáo —
              gói gọn trong 1 ứng dụng siêu mượt.
            </p>
            <ul className="space-y-2 text-sm">
              {['Thanh toán cực nhanh, hỗ trợ phím tắt', 'Tồn kho realtime, cảnh báo hết hàng', 'Dashboard biểu đồ trực quan'].map((t) => (
                <li key={t} className="flex items-center gap-2 text-slate-300">
                  <span className="size-1.5 rounded-full bg-gradient-to-r from-indigo-400 to-pink-400" />
                  {t}
                </li>
              ))}
            </ul>
        ...
        <div className="p-8 md:p-10">
          ...
          <div className="mt-6 text-center text-xs text-slate-400">
            Mẹo: Nếu Supabase project mới, hãy tắt <span className="text-slate-200">Confirm email</span> trong{' '}
            <span className="text-slate-200">Auth → Providers → Email</span> để đăng nhập ngay.
          </div>
        </div>
      </motion.div>
    </div>
